...
from edge_prop.models import SparseBaseline
from edge_prop.constants import NO_LABEL


class TestSparseBaseline(TestCase):
    def setUp(self) -> None:
        g = nx.Graph()
        ...
        self.edge_prop_model = SparseBaseline()
    ...
    def test_predict2(self):
        ...
        edge_prop_model = SparseBaseline()
    ...
    def test_predict3_2(self):
        ...
        edge_prop_model = SparseBaseline(max_iter=10_001)
        edge_prop_model.fit(graph, 'label')
        results = edge_prop_model.predict()
    ...
    def test_predict3(self):
        ...
        edge_prop_model = SparseBaseline(max_iter=5)
        edge_prop_model.fit(graph, 'label')
        results = edge_prop_model.predict()
        print(edge_prop_model.edge_distributions.todense()[:,:,0])
        self.assertListEqual(list(results), list(true_labels))
