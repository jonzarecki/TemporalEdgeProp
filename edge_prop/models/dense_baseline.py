...
class DenseBasline(BaseModel):
    ...
    def _perform_edge_prop_on_graph(self, adj_mat: np.ndarray, y: np.ndarray, max_iter=50,
        ...
        last_Y = adj_mat.dot(Y).sum(axis=1)  # TODO: axis 0 or 1 ?

        mat_sum = np.sum(last_Y, axis=-1, keepdims=True)
        ...
        # last_Y_edges = last_Y[:, np.newaxis, :] + last_Y[np.newaxis, :, :]
        # last_Y_edges[A == 0] = 0
        # mat_sum = np.sum(last_Y_edges[A != 0], axis=-1)[:, np.newaxis]
        # mat_sum[mat_sum == 0] = 1
        # last_Y_edges[A != 0] = last_Y_edges[A != 0] / mat_sum
        ...
        # save original labels - irrelevant at end
